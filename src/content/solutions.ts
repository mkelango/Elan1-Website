...
export const solutions: Solution[] = [
  {
    ...
    compliance:
      "Consent and residency are enforced in code rather than declared: a clinical write without an active patient consent is refused, and every interop call from outside the India region is refused at the connector with \"cross-border PHI is not allowed\". PHI minimisation is a store-side refusal matched over normalised field names, so aadhaarNumber, card_no and cvv are refused the same as the spellings someone happened to type. The interop layer is a governed seam and says which mode it is in: the FHIR gateway reads a live R4 endpoint when one is configured, while the ABDM HIE-CM consent manager and the X12 270/271, 837/835 and 278 flows are deterministic adapters over health1's own record — real fidelity, no external call. DISHA is carried as a recorded, audited compliance posture, not a gate; the gates are consent, residency, minimisation, the K5 transitions and the signing refusal. health1 is provider-side: it documents, codes, claims and measures — it does not adjudicate as a payer. None of this is a regulatory certification, and nothing here is medical, legal or financial advice.",
    outcomes: [
      "A clinical note whose text asserts a diagnosis, a prescription or a treatment decision has no path to signed — the refusal sits on the write path, so it holds for a human typist, for the pack's own advisor, and for a live model with a provider key.",
      ...
      "The eligibility verdict, the claim scrub status, the observation abnormality flag, the medication–allergy conflict and the quality-measure rate are recomputed on every write; a payload that supplies one does not get to keep it.",
      "A quality measure over an empty cohort has no publish path, so the insight it composes into insight1 cannot carry a rate over a denominator of zero.",
      "Every hand-off out of health1 is recomputed rather than asserted, and the receipt reports how many sibling records were scanned and how many could not be read — so \"clean\" and \"unexamined\" are different answers.",
    ],
    starterEngagement:
    ...
    regulatoryRails: [
      { name: "Patient consent (DPDP / ABDM / HIPAA framing) — the PHI gate", status: "enforced", note: "Six object types are PHI by declaration — encounter, clinical_note, condition, allergy, observation, medication — and both create AND update are refused without an ACTIVE consent for the resolved patient; a partial PATCH carrying no patient_id resolves the stored one. ALSO enforced on the read side: the consent gate 403s before reading anything, the advise endpoint refuses first, the population copilot skips per-patient, and the FHIR seam resolves the SUBJECT BY SHAPE for all three read ops rather than naming one." },
      ...
      { name: "ABDM HIE-CM consent manager", status: "modelled", note: "Adapter over health1's own SoR — no live HIE-CM connection. What IS enforced is the purity: the connector decides nothing and WRITES nothing; request_consent and revoke_consent return a governed write / a governed writes instructions the caller must route through health1's governed writer." },
      { name: "X12 270/271 eligibility · 837/835 claim · 278 prior-auth", status: "modelled", note: "the adapter reads health1's own coverage/claim records; there is no clearinghouse connection. What is enforced is that the derived facts are COMPUTED, not accepted: eligibility status is derived from the patient's coverage, claim scrub_status is 'clean' only if active coverage AND codes AND a positive amount (13342-13356), and a malformed claim cannot be SUBMITTED at all." },
      { name: "PHI minimisation (DPDP)", status: "enforced", note: "Store-side on every write. The classifier runs a seven-literal FLOOR (full_clinical_note, raw_lab_values, imaging_pixels, genomic_sequence, psychotherapy_notes, ssn_aadhaar_plaintext, payment_card) plus nine identity-qualified the minimisation list as the CEILING, matched over a NORMALISED key — so aadhaarNumber, card_no, cvv, bank_account_number and passport_no are refused, not only the one spelling someone typed, 371-417)." },
      ...
      { name: "HEDIS measure registry", status: "declared", note: "Reference data — four specs (CBP, CDC-HbA1c, BCS, CIS) at. Referencing a hedis_id fills name + condition from the registry instead of free text; the numerator/denominator/rate stay COMPUTED from the SoR's care plans on every write, and publishing over an empty cohort is REFUSED (13515-13527)." },
      ...
      { name: "Human sign-off (K5) on the consequential transitions", status: "enforced", note: "The clinical write path makes human-gated: ANY delete; encounter→finished; consent→revoked AND consent→active (both directions — opening the PHI gate is a person's decision); appointment→booked; claim→submitted; claim→paid; prior_authorization→submitted; prior_authorization→approved; clinical_note→signed; care_plan→active; quality_measure→published; service_request→active. The care-spine recomputes gated by calling this SAME function, so the console, the mark and the write path cannot disagree." },
    ],
    refusals: [
    ...
    evals: "SIX eval sets, NINE cases, and it can genuinely fail. Sets, mirrored exactly in pack.yaml so the pack door and the server door cannot issue two different marks): health1.grounding_accuracy (2 cases — a field check('grounded', required=True, measured by='the grounding measure') + a match rule over record/grounded/measure/fhir); health1.safety (1 — no_terms over the 12 a declared list, run against the tenant's SIGNED note summaries); health1.fairness (1); health1.phi_safety (1 — no_terms over the 7 the minimisation list names); health1.engine_never_acts_for_the_human (2 — an 8-phrase floor such as 'signed the note automatically' / 'submitted the claim without approval', PLUS the classifier check(the classifier) so a paraphrase the list never anticipated still fails); health1.phi_seam_clean (2 — a field check('phi_seam_clean', required, measured by='the seam measure') + a field check('handoffs_gated', required)).",
    seo: {
...

  {
    ...
    wedge: "Money can only leave on a human's signature, and the scheduled route around that signature is closed on the write path. The banking gate routes payment→executed, loan→disbursed, repayment→paid, deposit→closed, account→active and mandate→active to K5; the banking write path then refuses an execute whose beneficiary trips the live sanctions classifier, whose amount exceeds the balance RECOMPUTED from the transaction ledger, or whose customer is not KYC-verified. The part that matters most: running a standing instruction (NACH / UPI AutoPay) creates a payment hardcoded to status: \"draft\" through bank1's own governed writer — \"Automating the paperwork must never automate the authorization — otherwise the wedge ('a human authorizes every money movement') would have a scheduled bypass\".",
    problem: [
