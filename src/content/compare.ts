...
export const COMPARISONS: Comparison[] = [
  // ───────────────────────────────────────────────────────────────────────────────────────────
  {
    ...
    ourModel:
      `elan1 is the operating record for ${factValue("suiteApps")} business functions, not an agent layer over one. Each of the ${factValue("systemsOfRecord")} systems of record sits behind a governed writer, and identity resolution, policy evaluation, the human approval gate and the audit append happen there — in that order, whoever is calling. A person in a screen, an API client and an agent all arrive at the same gate rather than at three copies of it. The gate is not a workflow recorded beside the action: a consequential action stops, requests an approval carrying its own arguments, and on resume the gate re-checks that the token is approved, that it belongs to the same action, and that a content hash over the resumed payload matches the hash over the payload the reviewer saw — then consumes the token.`,
    dimensions: [
      {
