...
export const products: Product[] = [
  {
    ...
    outcomes: [
      ...
      "A forecast assembled from open deals, not from optimism",
      "An approval trail an auditor can follow end to end",
    ],
    integrations: [
      "The CRM seam — sales1's own system of record, not someone else's",
      "Email and calendar — a send is consent-checked and one-tap confirmed",
      "Voice and meeting capture — declared seams, not a live integration",
      "LinkedIn and WhatsApp — each with its own send-confirm",
      "Web research — public firmographic and news context",
      "Enrichment — each value cites its provider and never overwrites a human-set field",
      "finance1, supply1, project1, service1, market1 — governed sagas, not external APIs",
    ],
    suiteFit:
      "The demand hub, composing both ways: outbound into finance1, project1, supply1 and service1; inbound from market1 and service1. Every step keeps its own approval gate rather than inheriting the caller's.",
    trust:
...

  {
    ...
    outcomes: [
      "An answer you can trace to a document — and an uncited one visible as uncited",
      "A refund, an account change and a close that each carry an approver and an audit row",
      "Support figures computed from the case book at read, each disclosing its sample",
      "One desk behind the whole suite, so a dispute lands in a governed queue",
    ],
    integrations: [
      "The helpdesk seam — tickets, customer 360, orders, SLA status, notes, status changes",
      "The knowledge seam — retrieval that returns citations",
      "The CRM seam — read the account, update the record",
      "Email and WhatsApp — send and receive, both modelled adapters",
      "No connectors of its own: service1 consumes the shared fabric under least-privilege scopes",
      "In-suite: an approved refund posts a finance1 credit note, a churn signal reaches sales1",
    ],
    suiteFit:
      "The post-sale hub: outbound to finance1, sales1 and project1, inbound from sales1, people1 and supply1 — and every money leg terminates at finance1's own gate, because service1 never moves money. Vertical packs open cases here rather than build a second desk: bank1 disputes, insure1 grievances, gov1 citizen grievances, telco1 complaints, manufacture1 breakdowns, retail1 returns, realestate1 snags, energy1 outages, edu1 student support. That migration is per-vertical and is not uniform — edu1's support route still writes its case record directly.",
    trust:
      "The refusal is in the data layer, not only in the prompt: the sensitive-action and grounding refusals listed here are enforced at the record. On an approved refund, service1 posts a finance1 credit note idempotently rather than moving money itself. Publishing a knowledge article is a separate human decision. The Trust Mark is eval-gated across six named sets, and a mark that outlives its evals is auto-revoked. Two limits stated plainly: the grounding eval measures lexical support and the absence of invented figures, not entailment; and an honest \"I don't have a documented answer for that\" currently scores as uncited, which is recorded in the source rather than papered over.",
    workforce: {
      registered: 11,
      launchWave: 4,
