...
export const GLOSSARY: Term[] = [
  ...
  },
  {
    ...
    category: "The write path",
    usage: "elan1",
    def: "The single code path a write to a system of record is meant to travel: identity, then policy evaluation, then — where the action is consequential — a human approval, then the write, then the audit entry. The same path applies whether the write arrives from a person, an agent, or another app, which is why a sibling app cannot reach past a gate the owning app enforces. A refusal is not silent: in retail1 a sale price above the item's maximum retail price comes back blocked with its reason, appends a refusal event to the hash-chained audit, and writes no record.",
    why: "Read the limit with the mechanism. A test derives every mutating request handler from the tree — no hand-kept roster — and fails if a new one writes to a system of record directly; the legacy handlers that still do are pinned at a ceiling that may only fall, never rise. So \"every write is governed\" is a direction of travel with a ratchet on it, not a fact to accept today. The figure itself is deliberately not quoted here: a platform count belongs in the site's single source of counts, with its derivation and the date it was taken, and this one is not there yet.",
