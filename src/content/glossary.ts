...
export const GLOSSARY: Term[] = [
  // ——— The idea ———————————————————————————————————————————————————————————————————————————————
  {
    ...
    category: "The idea",
    usage: "narrowed",
    def: `Software that pursues a goal over several steps, picks its own tools along the way, and finishes by writing to a record. On elan1 an agent is a typed specification held in a central registry — a name, an instruction, a model policy, a tool grant, its policy tags, an autonomy tier, and whether it verifies itself before writing back — rather than a prompt somebody pasted into a box. Across the ${SUITE_APPS} suite apps ${SUITE_AGENTS} agents are declared and ${SUITE_LAUNCH_WAVE} are enabled in the launch wave; platform-wide the registry holds ${factValue("agentsRegistered")} with ${factValue("agentsEnabled")} enabled.`,
    why: "We use the word more narrowly than the industry does. If it only produces text there is nothing to govern — every other mechanism in this glossary exists because the last step touches a record.",
    see: ["Blueprint", "Autonomy tier", "Governed writer"],
  },
  {
    term: "Agentic transformation",
    category: "The idea",
    def: "Adopting agents as an operating layer for the business rather than as a feature inside an existing app — the work, the record it writes to, and the control over that write all moving together.",
    why: "Our version of it is deliberately unglamorous: a sequence of functions enabled one at a time, each with a gate, on a control plane that refuses the ones you have not enabled yet.",
  ...
  },
  {
    ...
    category: "The idea",
    usage: "narrowed",
    def: "An assistive surface inside a screen: it reads, computes and suggests, and a person does the thing. On the product pages each copilot carries one of two labels — advisory, meaning it returns a result and writes nothing, or acts-through-a-gate, meaning it proposes and the owning app's approval gate applies before any write.",
  ...
  },
  {
    ...
    category: "The idea",
    usage: "elan1",
    def: "A declaration that a thing recommends and never acts. In agent1 it is enforced when the agent is built rather than by convention: a blueprint marked advisory may hold none of the platform's control-system connectors, and the compiler raises instead of building it. Its instruction is stamped with the advisory-only line, and stamped the same way every time so republishing does not churn the content hash and silently revoke its Trust Mark.",
  ...
  },
  {
    ...
    category: "The idea",
    usage: "elan1",
    def: "The declared level an agent runs at: autonomous (log, enrich, note), one-tap (a lightweight human confirm), or human-led (a person decides). Read this precisely — the tier is declarative metadata, and what actually gates an action is the spec's requires-approval flag plus the policy evaluation. Raising an agent's tier does not open a consequential action: in insight1, publishing stays human-approved at every level. Where a tenant opts into graduated autonomy, a small in-tolerance action can proceed without a gate, anything above the tolerance or over the per-period budget escalates to a human, and a value above a hard ceiling is refused outright; an unconfigured action class stays human-approved. An agent cannot widen its own envelope.",
    why: "This is the honest version of \"autonomous agents\": autonomy is granted by a person, bounded by a budget that spends down, and never covers the consequential end.",
  ...
  },
  {
    ...
    category: "The write path",
    usage: "elan1",
    def: "The single code path a write to a system of record is meant to travel: identity, then policy evaluation, then — where the action is consequential — a human approval, then the write, then the audit entry. The same path applies whether the write arrives from a person, from an agent, or from another app, which is why a sibling app cannot reach past a gate the owning app enforces. A refusal is not silent: in retail1 a sale price above the item's maximum retail price comes back blocked with its reason, appends a refusal event to the hash-chained audit, and writes no record.",
    why: "Read the limit with the mechanism. A test in the platform derives every mutating request handler from the tree — no hand-kept roster — and fails if a new one writes to a system of record directly; the legacy handlers that still do are pinned at a ceiling that may only fall, never rise. So \"every write is governed\" is a direction of travel with a ratchet on it rather than a fact to accept today. The figure itself is deliberately not quoted in this definition: a platform count belongs in the site's single source of counts, with its derivation and the date it was taken, and this one is not there yet.",
