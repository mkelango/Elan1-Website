...
const INSIGHT_SOURCES: InsightSource[] = [
  {
    ...
    abstract:
      "One question separates a feature from an operating layer: after this ships, which record changes without a person typing into a form, and whose name is on that change? Agents read your own records, propose the next move, and any change travels one governed path — identity, policy, a named human on consequential actions, a hash-chained audit. Enablement is per tenant and per function. Two limits: strategy1 is a delivery motion performed by people — no application, agent, screen or endpoint — and nothing here has been measured in a customer environment.",
    body: [
      {
        h: "The feature trap",
...

  {
    ...
    abstract:
      "Strip the abstraction out of most agent debates and one question is left: may this thing send. elan1 gives agents the ninety-nine and holds the one. service1's resolution agent drafts; a person sends. Self-service answers only above a confidence floor. In market1, publishing and sending are consequential by declaration, so the runtime routes them to a person even when the agent never thought to ask. One limit: the assistant's channel connectors are modelled sends in this build — recorded rather than dispatched — until channel credentials are configured.",
    body: [
      {
...

  {
    ...
    abstract:
      "There are two places to put a control on an agentic system, and they are not substitutes. The first is the call: scope checked at call time, consequential operations gated, the call audited by its operation and argument names rather than its values. The second is the write. The difference is arity: a tool call is one path into a record, a person in a screen is another, a sibling application composing a workflow is a third, and a control at the write is the one all three share. A read is not on that path — a statement about approval and audit, not about access. And the honest part: a guard freezes the count of mutating handlers still reaching a store directly as a ceiling that may fall and may never rise.",
    body: [
      {
...

  {
    ...
    abstract:
      "Most approval workflows are attestation: somebody clicks approve, a row is written, and the action proceeds down a code path that never reads the row again. elan1 binds them. On resume the gate checks three things — approved, same action, and a content hash over the resumed payload matching the approved one — then consumes the token. So an approval for one operation cannot be used on another, and a replayed token is already spent. The id comes from the caller, which is precisely why the gate cannot treat the id as the check. The earlier defect was a gate that asked only whether an approval was approved; it is closed, and closed identically in both gates. Under maker-checker the requester cannot approve their own request, and the administrator role does not bypass it.",
    body: [
      {
...

  {
    ...
    abstract:
      "A written policy has no failure mode: it reads the same on the day it is enforced and the day the enforcement is removed. A refusal has none of that slack — it exists because a live path declined a specific write, names the reason in the language of the business, and lands in the audit beside the record it protected. A sale price above a product's maximum retail price is not persisted; an unbalanced journal is refused with its own debit and credit totals; a discount with no list price behind it is refused as ungrounded; a standing payment mandate is drafted for approval rather than executed. And a refusal is deliberately distinguishable from a failure. Ask to see the refusal, not the policy.",
    body: [
      {
...

  {
    ...
    abstract:
      "Registered, enabled and running are three different words, and a catalogue number that blurs them does a reader a disservice. elan1 publishes the first two rather than the larger one alone. A function outside the tenant's set is refused by name — application, function and tenant. A separate suspend switch overrides the entire enabled set and identifies itself as an incident kill-switch. What it stops is the agent fleet: it does not reach that application's direct writes to its own system of record, and this piece says so in the present tense. Reconciliation runs both ways, and an unloaded registry reports not measurable rather than zero per cent.",
    body: [
      {
...

  {
    ...
    abstract:
      "The blocking question in an agent programme is almost never capability. It is evidence. Four mechanisms carry it: an approval token bound to the action and to a content hash of its exact payload, then consumed on use; a Trust Mark refused on a run that scored zero cases, one that did not pass, and one belonging to another tenant; an audit hash-chained per tenant, with updates and deletes blocked by a database trigger; and enablement staged per tenant. Then the part that makes the rest credible: the default agent certification is a structural declaration check, and the behaviour battery covers a named handful of agents rather than the whole roster.",
    body: [
      {
...

  {
    ...
    abstract:
      "assure1 ships no package of its own — its code is the certification kernel inside the core, because certification beside the runtime is one a caller can forget to consult. Three refusals define the mark: a run that scored zero cases cannot mint one, a run that did not pass cannot mint one, and a run belonging to another tenant cannot mint one. The mark is bound to a content hash of the definition it certifies, so editing the agent revokes it. Now the part usually left out: the tier is reported as structural, behavioural or adversarial, the default is structural, and the behaviour battery covers a named handful of agents rather than the whole roster. Read the tier, not the badge.",
    body: [
      {
...

  {
    ...
    abstract:
      "A two-valued verdict has a structural bug: absence collapses into the good answer. A check that ran zero cases reports no failures. A boolean over an empty set is true. An application with no eval suite has never failed one. elan1 carries a third verdict — passed, failed, not measurable — and treats it as a refusal at a gate rather than a skip. An application with no suite, a suite that has never run here, and a mistyped application id all report not measurable rather than clearing the gate, which the last of those previously did; a test now asserts exactly that. The discipline has to reach the surfaces too, or it is only a type. This was paid for: a gate once returned a pass for an application that had no eval suite at all, and the comment beside it called that honest.",
    body: [
      {
...

  {
    ...
    abstract:
      "Two obligations that look contradictory: an audit trail that cannot be edited, and a data principal who can require erasure. They are reconciled by layer. A data-subject erasure anonymises the personal fields, keeps the record shell and its id so referential integrity and audit linkage survive, marks the record erased, and appends its own erasure event. The hash-chained audit is retention-exempt and never rewritten. Alongside that, some paths log a shape rather than a value: a CRM write records the field keys it touched and not their contents. That is a discipline at specific boundaries, not a property of every event — other governed actions do record business values, and a governed send records its recipient and subject line. Immutability is enforced at the database: a trigger raises on update and delete including for the table owner, and a unique constraint on the tenant and the previous hash means the chain cannot fork.",
    body: [
      {
...

  {
    ...
    abstract:
      "The integration tax is usually described as plumbing, but the expensive part is governance: each tool arrives with its own identity model, its own idea of an approval, and its own audit, so any control that spans them is re-proved at every boundary. elan1 puts identity, policy, approvals, audit, telemetry, the skills registry and the connector fabric in one core and builds thin applications on it. An industry is then a configuration pack rather than a branch of an application's source. What makes that credible is the composition rule: a pack that needs a service case opens one through service1's own governed writer and raises an error if service1's gate refuses. Each hand-off keeps the target's gate rather than the caller's. And a pack adds only what no horizontal application owns, while deliberately not rebuilding case resolution, receivables ageing or satisfaction scoring.",
    body: [
      {
...

  {
    ...
    abstract:
      "A connector can be in three quite different states, and a logo grid renders all three identically. Registered and callable means the runtime can invoke it, scoped, gated and audited. Modelled means that call reaches a deterministic adapter with the real interface and no external system behind it — the assistant's channel connectors are modelled sends in this build, and the web reference connector is a deterministic mock. Declared means a typed seam exists and nothing runs until credentials and an audited grant are in place. We label the state on each entry because the governance is identical across all three and the connection is not. A modelled adapter is not a mock in the dismissive sense: the interface, the scopes, the refusals, the least-privilege grant and the audit are all real, and the data is synthetic. Treat an unwired capability as scope to agree, not as a feature to enable.",
    body: [
      {
...

  {
    ...
    abstract:
      `Stored aggregates drift because they are written once, by a path that can fail, and never re-derived. The sharper failure is a metric that reads a field nothing populates and returns a confident zero — a structural zero wearing a metric's name is indistinguishable on a dashboard from good news. elan1 computes at read and stores almost nothing. insight1's values are computed from the ${factValue("insightSourceApps")} sibling source stores its semantic layer names, at the moment you run them. That set is a named list rather than "every application": goal1 is not in it, and all but one of the rest are registered systems of record — the exception is customer1's CRM on its own core store. insight1 also holds no write access to another application's system of record. And a computed figure must disclose the sample it rests on, or it is not evidence.`,
    body: [
      {
...

  {
    ...
    abstract:
      "The most reliable way to ship a false statement is to type a true one and wait. This site has done it: a hand-typed figure whose entire job was to establish credibility understated the product — in both directions at once, on different lines — for as long as nobody re-counted. Diligence does not fix that, because the statement was diligent when it was written. The remedy is structural. This site keeps one module of platform counts, each carrying its derivation and the date it was counted; a page imports the number rather than typing it, and a count with no derivation does not belong in the file. Where three derivations of one count produced three different answers, the count was removed rather than published. A figure that only a deployment can settle is published as a dated, attributed reading. And a check derived from the very collection it describes cannot fail.",
    body: [
      {
...

  {
    ...
    abstract:
      "A clean bill is unfalsifiable, and it discounts everything above it the moment a reader finds one thing it missed. elan1 publishes the open items instead, in three forms. First, state labelling: each engineering control is published as enforced, declared, or built-but-not-wired, and none is promoted to enforced without a production caller to point at. Second, a named limits list — residency declared and not enforced by routing, no SOC 2 or ISO 27001 certification held today, no third-party penetration-test report on file, no run1 service-level-agreement machinery, the incident kill-switch stopping an application's agent fleet and not its direct writes to its own system of record, the resilience logic real while the multi-node deployment is not, and the capabilities called by nothing in production named individually. Third, the ratchet: a guard freezes the count of mutating handlers reaching a store directly as a ceiling that may fall and may never rise. So \"every write is governed\" is published as a direction with a ratchet on it rather than as a fact.",
    body: [
      {
