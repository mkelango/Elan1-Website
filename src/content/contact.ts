...
export const WHAT_HAPPENS_NEXT: ContactStep[] = [
  {
    ...
    who: "You",
    body: "It becomes one record with your consent attached, and those details are used to reply to this request and follow up on it. We do not publish a response time here. It is the promise a website breaks most often, we have not measured ours, and printing a number nobody has measured would be the first thing we got wrong.",
  },
  {
    n: "02",
    title: "A conversation with the product open",
    who: "elan1 people",
    body: "Not a pitch. We open the platform and drive the parts that are awkward as well as the parts that demo well: a consequential action stopping at a named human, an audit chain re-hashed as it is read rather than shown from a stored verdict, and a function nobody has enabled being refused before it acts — “finance1.ap_ar not enabled for tenant acme”. Bring your workflow and we will hold it against that.",
    link: { label: "Or run a governed agent yourself, now", href: "/demo" },
  },
  {
    n: "03",
    title: "A Discovery Sprint, if that is the right next step",
    who: "elan1 people — strategy1",
    body: "strategy1 is a delivery motion, not software: interviews, workshops and a read of your current systems, done by consultants. There is no strategy1 app, agent or screen, and no part of this step is an automated scan. The scope is fixed before it starts; the fee, and how it credits toward the Launchpad that follows, are on the pricing page.",
    link: { label: "How strategy1 runs a Discovery Sprint", href: "/platform/strategy1" },
  },
  {
    n: "04",
    title: "What leaves that engagement",
    who: "You hold it",
    body: "A document set and a rollout order — not a running system. An opportunity map, a sequenced roadmap, a value model built from your own operating assumptions and labelled illustrative because nothing has been measured in your environment, an operating model written against gates that already exist, and a written list of what the engagement did not answer, carried forward rather than implied closed. The sequence is worth more than an ordinary roadmap for one reason: agent functions are enabled per tenant, one at a time, so “not this wave” becomes a state the control plane holds rather than a bullet you have to trust.",
  },
  {
    n: "05",
    title: "A first agent is scoped separately",
    who: "elan1 — agent1, gated by assure1",
    body: "A working agent is its own build, not an automatic inclusion in the sprint. When it is scoped it reaches the runtime the way every agent does: publish is refused without a passing eval for that agent in that tenant, publishing is itself a human approval, and the Trust Mark mints before the deploy runs. We will not put a date on the mark. It is eval-gated and may withhold — a run that scored zero cases, one that did not pass, and one belonging to another tenant are each refused.",
    link: { label: "What a Trust Mark is gated on", href: "/platform/assure1" },
  },
  {
    n: "06",
    title: "Then it is operated, and widened one function at a time",
    who: "You, or run1 alongside you",
    body: "Runs and traces read from the same hash-chained record as the audit, so there is no second log to reconcile against the governance one. Cost is priced from the platform’s own rate card rather than reported by the model. Expanding is another enablement, and the same lever run backwards is how it narrows again. If you would rather not staff the operating seat, elan1 people take it through the same governed paths your own operators use.",
