...
export const NOT_CLAIMS: NotClaim[] = [
  ...
  },
  {
    ...
    weAreNot:
      "elan1 is not a system you can set to run consequential actions unattended. The default for every action class is a human approval, nothing in the shipped configuration changes that, and removing the gate is not a feature we have not got round to — the gate is the product, and a version of elan1 without it would be a different product with our name on it. One qualifier belongs here rather than in a footnote, because a reader who went looking would find it: a graduated-autonomy mechanism exists in the platform. It lets a tenant opt ONE action class in to proceeding without approval inside a value tolerance and a spend budget — above the tolerance it escalates to a human, above a hard ceiling it is refused outright, and every path is audited. It is built and it is not wired into the shipped composition root, so nothing runs on it today and no deployment can switch it on from a screen. Built and not wired is what it is, and that is what we call it.",
    whatWeAre:
      "Waiting never opens a gate. A pending decision that breaches its service level escalates and never decides — there is deliberately no auto-approve on that path, because a gate that opens itself when people are slow converts a backlog into unreviewed consequential action exactly when attention is scarcest. An approval is consumed on use, bound to the exact action string, and fingerprinted to a content hash of the payload the reviewer actually saw, so a yes given for one operation cannot be spent on another or on an edited version of the same one. Where a writer stamps an action maker-checker — releasing a payment, posting a journal entry, a refund — the person who requested it may not approve it, verbatim from the approval gate: \"segregation of duties — {principal} requested approval '{id}' and cannot also approve it; a different approver is required\" — and an administrator's role does not lift that, because it is a control rather than a permission. The scope is worth stating exactly: that stamp is on the money actions, not on every consequential one, and an unstamped approval can be resolved by anyone holding the approver role. A decision returned too fast to have been read is surfaced as exactly that, computed from the audit chain rather than stored as a flag someone could clear.",
    why:
      "State the cost plainly: on any workflow whose bottleneck is a person's attention, elan1 will be slower than an ungoverned agent, and the mechanism that would narrow that gap is bounded and opt-in by construction rather than a switch that removes the person. If your goal is an unattended pipeline that moves money, publishes to an audience, or touches a network, we are the wrong platform and you should stop evaluating us here. That is a real segment of the market and we are deliberately not in it.",
    accent: ACCENT.rose,
  },
  {
    ...
    why:
      "Two limits inside this one, since they are what a serious buyer will ask next. Residency is declared per tenant and its data classification is fail-closed — an undeclared field is refused rather than waved through — but the declaration is not enforced by routing, and the platform publishes it as exactly that pair: declared true, enforced false. The multi-region router exists and is exercised as a demonstration against the real tenant map; nothing on the serving path consults it. Single-region today, and the honest word for the residency line on your questionnaire is declared. And we publish no latency or throughput figure, because none has been measured under a customer's load — a benchmark run against our own fixtures would tell you about our fixtures. The first production deployment will be somebody's first. We would rather you knew that going in than discovered it in month three.",
  ...
  },
  {
    ...
    whatWeAre:
      "Claude is the native provider — its own client, its own API — and it is what the platform is built, tested and evaluated against. The model seam is genuinely open beneath that: one shared client serves the OpenAI-compatible providers, and OpenAI, NVIDIA NIM, Kimi/Moonshot and GLM are configured seams, each needing its own credential — with a local Ollama daemon the keyless one. The seam is real. The equivalence is the part we will not assert.",
    why:
