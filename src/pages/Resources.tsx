// pages/Resources.tsx — Help Center, Community, Developers, Academy/Courses
...
const cards = [
  ...
  { category: "Help Center", t: "Playbooks", b: "Step-by-step guides from discovery to production and scale.", href: "/resources/playbooks", accent: "#df8c64", live: true },
  { category: "Help Center", t: "Templates", b: "Pre-built agent templates for common enterprise workflows.", href: "/resources/templates", accent: "#5ad1c0", live: true },
  ...
  { category: "Community", t: "Case Studies", b: "Real customer success stories with measurable outcomes.", href: "/resources/case-studies", accent: "#3fae6b", live: true },
  ...
  // Academy/Courses
  { category: "Academy", t: "Training", b: "Self-paced training modules and certification programs.", href: "/resources/academy", accent: "#7c6cf0", live: true },
  { category: "Academy", t: "Certification", b: "Professional certifications for building and operating agents.", href: "/resources/academy", accent: "#7c6cf0", live: true },
  { category: "Academy", t: "Bootcamps", b: "Intensive workshops and hands-on training events.", href: "/resources/academy", accent: "#7c6cf0", live: true },
];

export default function Resources() {
  useSeo("Resources | elan1", "Learn, build, connect, and master: guides, documentation, events, APIs, and training.");
  ...
  const academy = cards.filter(c => c.category === "Academy");
  ...
  return (
    <>
      <PageHero
        kicker="Resources"
        accent="#7c6cf0"
        title="Learn, build, connect, master."
        subtitle="Documentation, templates, playbooks, case studies, community, APIs, and training—everything you need."
        cta={{ label: "Browse guides", href: "/resources/help-center", secondary: { label: "Book a demo", href: "/contact" } }}
      />
      
      {/* Help Center */}
      <Section tone="paper">
        <SectionHead title="Help Center" sub="Guides, whitepapers, playbooks, and templates" />
      ...
      {/* Community */}
      <Section tone="white">
        <SectionHead title="Community" sub="Events, webinars, case studies, and customer stories" />
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mt-8">
          {community.map((c, i) => (
            <Reveal key={c.t} delay={(i % 3) * 0.06}>
              <Link to={c.href} className="group flex h-full flex-col rounded-card border border-line bg-surface p-7 shadow-card transition-all hover:-translate-y-1 hover:shadow-lift">
      ...
      {/* Developers */}
      <Section tone="paper">
        <SectionHead title="Developers" sub="APIs, SDKs, and connectors for building agents" />
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mt-8">
          {developers.map((c, i) => (
            <Reveal key={c.t} delay={(i % 3) * 0.06}>
              <Link to={c.href} className="group flex h-full flex-col rounded-card border border-line bg-surface p-7 shadow-card transition-all hover:-translate-y-1 hover:shadow-lift">
      ...
      {/* Academy/Courses */}
      <Section tone="white">
        <SectionHead title="Academy" sub="Training programs, certifications, and bootcamps" />
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mt-8">
          {academy.map((c, i) => (
            <Reveal key={c.t} delay={(i % 3) * 0.06}>
              <Link to={c.href} className="group flex h-full flex-col rounded-card border border-line bg-surface p-7 shadow-card transition-all hover:-translate-y-1 hover:shadow-lift">
