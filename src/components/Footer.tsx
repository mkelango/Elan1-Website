...
const cols: { heading: string; links: { label: string; href: string }[] }[] = [
  {
    ...
    links: [
      ...
      { label: "customer1", href: "/products/customer1" },
        ...
        <div className="grid gap-10 lg:grid-cols-[1.4fr_3.6fr]">
          <div>
            ...
            <div className="mt-6 flex gap-3 text-slate">
              {["in", "x", "yt"].map((s) => (
                <a key={s} href="#" aria-label={s} className="flex h-9 w-9 items-center justify-center rounded-full border border-line text-xs font-mono uppercase hover:border-ink/40 hover:text-ink">
                  {s}
                </a>
              ))}
            </div>
